...
describe('SidebarApp', () => {
  ...
  it('initializes showBeta to false when localStorage explicitly set to "false"', async () => {
    const user = userEvent.setup();
    window.localStorage.setItem('nia_show_beta', 'false');
  ...
  it('handles completely empty versions object from registry', async () => {
    // Mock response with empty versions object
    const response = { ok: true, json: async () => ({ versions: {} }) } as Response;
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(response));

    render(<SidebarApp packageName="empty-package" />);

    // Should only have "latest" option, no versions loaded
    await waitFor(() => {
      expect(screen.getByRole('option', { name: 'latest' })).toBeInTheDocument();
    });
  ...
  it('verifies all state updates on network error', async () => {
    const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
    ...
    // Version dropdown should exist with only latest
    const versionSelect = screen.getByTestId('version-select');
    expect(versionSelect).toBeInTheDocument();
    // Query options within the version select specifically
    const versionOptions = versionSelect.querySelectorAll('option');
    expect(versionOptions).toHaveLength(1);
    expect(screen.getByRole('option', { name: 'latest' })).toBeInTheDocument();
  ...
  it('cancels pending fetch on unmount', async () => {
    ...
    const { unmount } = render(<SidebarApp packageName="react" />);
    ...
    // Resolve the promise after unmount - should not cause any state updates
    // If cancellation works, this won't trigger any errors or state changes
    resolvePromise?.({ ok: true, json: async () => createRegistryResponse(['1.0.0']) } as Response);

    // No assertions needed - test passes if no errors thrown
  });

  it('deduplicates version options when filteredVersions contains duplicates', async () => {
    // This tests the deduplication logic at line 265
    // We need to create a scenario where filteredVersions might have 'latest' in it
    mockFetch(['2.0.0', '1.0.0']);
    ...
    // Verify version options are present without duplicates
    const versionSelect = screen.getByTestId('version-select');
    const options = versionSelect.querySelectorAll('option');

    // Should have: latest, 2.0.0, 1.0.0 (no duplicates)
  ...
  it('handles maxVersions input with non-numeric value', async () => {
    const user = userEvent.setup();
    render(<SidebarApp packageName="react" />);
    await screen.findByRole('option', { name: '2.0.0' });
  ...
  it('handles maxVersions input with negative value', async () => {
    const user = userEvent.setup();
    render(<SidebarApp packageName="react" />);
    await screen.findByRole('option', { name: '2.0.0' });
    ...
    // Enter negative value using fireEvent for direct value change
  ...
  it('handles maxVersions input with decimal value', async () => {
    const user = userEvent.setup();
    render(<SidebarApp packageName="react" />);
    await screen.findByRole('option', { name: '2.0.0' });
    ...
    // Enter decimal value using fireEvent for direct value change
    fireEvent.change(maxInput, { target: { value: '5.7' } });

    // Should floor to 5
    expect(maxInput).toHaveValue(5);
  });

  it('caps maxVersions input at 99', async () => {
    const user = userEvent.setup();
    render(<SidebarApp packageName="react" />);
    await screen.findByRole('option', { name: '2.0.0' });
  ...
  it('keeps settings dropdown open when clicking inside', async () => {
    const user = userEvent.setup();
    render(<SidebarApp packageName="react" />);
    await screen.findByRole('option', { name: '2.0.0' });
    ...
    // Click inside the settings dropdown (on the checkbox label)
    const checkbox = screen.getByTestId('show-beta-checkbox');
  ...
  it('verifies textarea cleanup in fallback copy mechanism', async () => {
    // Ensure clipboard is undefined to trigger fallback
    const originalNavigator = global.navigator;
    vi.stubGlobal('navigator', { ...originalNavigator, clipboard: undefined });

    // Mock document methods for textarea manipulation
    ...
    // Verify textarea was added and then removed
  ...
  it('covers the explicit false branch for showBeta initialization', async () => {
    // This test specifically targets line 140-141 (else if branch)
    window.localStorage.setItem('nia_show_beta', 'false');

    render(<SidebarApp packageName="react" />);
    await screen.findByRole('option', { name: '2.0.0' });

    // The component should have explicitly set showBeta to false
    // We verify this by checking beta versions are hidden
    expect(screen.queryByRole('option', { name: '1.1.0-beta.1' })).not.toBeInTheDocument();
  });
